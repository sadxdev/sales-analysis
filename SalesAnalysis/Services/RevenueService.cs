...
namespace SalesAnalysis.Services
{
    ...
    {
        Task<object> GetTotalRevenueAsync(DateTimeOffset startDate, DateTimeOffset endDate);
        Task<object> GetRevenueByProductAsync(DateTimeOffset startDate, DateTimeOffset endDate, int top = 50);
        Task<object> GetRevenueByCategoryAsync(DateTimeOffset startDate, DateTimeOffset endDate);
        Task<object> GetRevenueByRegionAsync(DateTimeOffset startDate, DateTimeOffset endDate);
    }
    ...
    public class RevenueService : IRevenueService
    {
        ...
        public RevenueService(SalesDbContext db, IConnectionMultiplexer redis, ILogger<RevenueService> logger)
        {
            _db = db;
        ...
        private string CacheKey(string prefix, DateTimeOffset startDate, DateTimeOffset endDate)
            => $"{prefix}:{startDate:yyyy-MM-dd}:{endDate:yyyy-MM-dd}";

        public async Task<object> GetTotalRevenueAsync(DateTimeOffset startDate, DateTimeOffset endDate)
        {
            ...
            var result = new { startDate, endDate, itemRevenue, shipping = shippingSum, total };
            await _cache.StringSetAsync(key, JsonSerializer.Serialize(result), TimeSpan.FromMinutes(30));

            return result;
        }

        public async Task<object> GetRevenueByProductAsync(DateTimeOffset startDate, DateTimeOffset endDate, int top = 50)
        {
            var key = $"{CacheKey("revenue:byproduct", startDate, endDate)}:top{top}";
            var cached = await _cache.StringGetAsync(key);
            if (cached.HasValue) return JsonSerializer.Deserialize<object>(cached);
        ...
        public async Task<object> GetRevenueByCategoryAsync(DateTimeOffset startDate, DateTimeOffset endDate)
        {
        ...
        public async Task<object> GetRevenueByRegionAsync(DateTimeOffset startDate, DateTimeOffset endDate)
        {
